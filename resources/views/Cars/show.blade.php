...
@section('content')
            ...
            <div class="w-2/3">
                ...
                <div class="grid grid-cols-4 grid-rows-2 w-full  gap-6 py-4   mt-8">
                    ...
                    <div class="px-4 py-2 border-2 col-span-2 rounded-lg relative">
                        ...
                        <x-star-rating :rating="3.4"/>
                        <p class="text-sm text-slate-500 text-center">out of {{count($car->reviews)}} reviews</p>
                        </p>
                    </div>
                </div>
                ...
                <div class="py-4 flex gap-4">
                    @auth('web')
                        ...
                        <a class="btn flex-grow text-center" href="{{route('reviews.create', $car)}}">Add a review</a>
    ...
    <div class="mt-12 px-10 py-6">
        <h2 class="text-center text-3xl font-semibold text-blue-700">
            Reviews of {{$car->model}}</h2>
        @forelse($car->reviews as $review)
            <div class="w-7xl flex items-center justify-center gap-2 px-4 py-5 border-2 rounded my-6 text-center relative">
                <div class="flex flex-col items-center">
                    <div class="w-12 px-4 bg-[url({{asset('assets/userAvatars/'.$review->user->avatar)}})] h-12 bg-center bg-cover border flex items-center justify-center rounded-full">
                ...
                <p class="text-[12px] text-right text-slate-600 absolute top-4 right-10">🗓 {{$review->created_at->format('M j, Y')}}</p>
            </div>
        @empty
            <p>No reviews yet....</p>
        @endforelse
